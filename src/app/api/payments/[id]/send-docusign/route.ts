...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Use plain object for envelope definition if EnvelopeDefinition is not available
  ...
  if (!accountId) {
    return NextResponse.json({ error: 'DOCUSIGN_ACCOUNT_ID is not set in environment variables.' }, { status: 500 });
  }
