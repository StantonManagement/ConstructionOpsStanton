...
async function runMigration() {
  log('🚀 Attempting to apply Migration 006 (Update Location Stats View)...', colors.cyan);
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '..', 'database-migrations', '006_update_location_stats_view.sql');
    const migrationSql = fs.readFileSync(migrationPath, 'utf8');

    log('📝 Reading migration file: 006_update_location_stats_view.sql', colors.cyan);
