...
async function runMigrations() {
  try {
    ...
    const migrations = [
      ...
      {
        name: 'schedule_defaults indexes',
        sql: `
      ...
      },
      {
        ...
        sql: `
          ...
          SELECT 'Stanton Management'
          WHERE NOT EXISTS (SELECT 1 FROM company_settings);
        `
      },
      {
        name: 'integration_credentials table',
        sql: `
      ...
      {
        name: 'daily_log_responses table',
        sql: `
      ...
      {
        name: 'punch_list_categories data',
        sql: `
          INSERT INTO punch_list_categories (name, display_order) VALUES
            ('Drywall', 10),
            ('Paint', 20),
            ('Flooring', 30),
            ('Electrical', 40),
      ...
      {
        name: 'punch_list_comments table',
        sql: `
    ...
    if (failCount === 0) {
      ...
      console.log('🎉 Database is now ready. Auto-schedule API should work.');
    }
