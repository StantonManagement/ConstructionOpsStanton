...
const SUPABASE_PROJECT_REF = 'iyiqdgmpcuczzigotjhf';
...
async function runMigration() {
  console.log('🚀 Starting Migration 007 via Supabase API...\n');
  ...
  try {
    ...
    if (portfolioError) {
      console.error('⚠️  Could not verify portfolios:', portfolioError.message);
    } else {
      console.log(`\n✅ Portfolios created: ${portfolios.length}`);
      portfolios.forEach(p => console.log(`   - ${p.name} (${p.code})`));
    }
    ...
    if (projectError) {
      console.error('⚠️  Could not verify projects:', projectError.message);
    } else {
