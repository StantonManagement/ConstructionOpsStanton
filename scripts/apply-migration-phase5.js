...
async function runMigration() {
  log('🚀 Attempting to apply Phase 5 Migration (Cash Flow)...', colors.cyan);
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '..', 'database-migrations', '004_create_cash_flow_tables.sql');
    const migrationSql = fs.readFileSync(migrationPath, 'utf8');

    log('📝 Reading migration file: 004_create_cash_flow_tables.sql', colors.cyan);
